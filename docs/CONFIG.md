...
| `params.force_stream` | bool | `false` | Sends `stream: true` on the wire whatever the caller asked, for a host that refuses anything else. **The caller's shape does not change**: a non-streaming caller still receives one buffered answer, because dorang reads the event stream back and collects it into the neutral form, which then goes through the same served-model check, tool-argument check, §10.5b transform and client encoder as a JSON answer. Only the ChatGPT Codex surface (`kind: codex-responses`) needs it in this build's catalog | Accepted **only** on a kind the catalog declares `responses_only: true`. On any other kind it is a **start-up refusal** naming the key and the provider: the chat adapter never reads it, so it would load and change nothing (§23.2). A collected stream that ends without its terminal event is a `502 upstream_stream_truncated`, a mid-stream error event is a `502 upstream_stream_error` with the upstream's text in `native_message` (credential scrubbed), and neither is offered to the fallback chain — the host bills the generation it began. A stream with no events at all is `upstream_shape` and IS offered to the chain, because nothing was generated |
| `params.store_false` | bool | `false` | Sends `store: false` on the wire unconditionally, for a host that refuses a stored exchange. Codex refuses `store` absent or true with `400 Store must be set to false` | Same rule as `force_stream`: accepted only on a `responses_only` kind, refused at start-up elsewhere |
| `params.api_version` | string | `""` | Azure OpenAI only. Selects the **legacy** per-deployment surface — `/openai/deployments/{deployment}/…?api-version=<value>` — and is its mandatory query parameter. Unset takes the unified `/openai/v1` surface, where the deployment name travels in the body's `model` exactly as the OpenAI encoder puts it | Refused at start-up on any kind but `azure` (and its aliases): only the Azure adapter reads it. The credential goes in the `api-key` header on either surface; an OAuth (Entra ID) credential travels as a bearer through the credential table untouched. Not measured against a live resource; Azure's published contract, pinned against a fake |
| `params.project`, `params.location` | string | `""` | Vertex AI only, both required there: the route is `/v1/projects/{project}/locations/{location}/publishers/google/models/{deployment}:generateContent` and there is no default for either. `base_url` may be left unset — the location names the host, `https://{location}-aiplatform.googleapis.com` (`global` → `https://aiplatform.googleapis.com`) | Refused at start-up on any kind but `vertex` (and `vertex-ai`). The credential is a Google OAuth bearer — a service-account key file through `auth: oauth` with `format: gcp-service-account` (below) — or an API key sent as `x-goog-api-key`. Gemini models only; `anthropic-vertex` is refused with the reason. Not measured against a live project; Google's published route, pinned against a fake |
| `params.region`, `params.access_key_id`, `params.session_token` | string \| string \| secret ref | `""` | Amazon Bedrock only. The SigV4 signing inputs: `region` and `access_key_id` are required (region may instead come from `base_url`, `bedrock-runtime.{region}.amazonaws.com`), and `session_token` is the optional short-lived STS token — a secret reference like any other (`key_env`/`key_file`/`key_ref`), never a literal. The **secret access key is the credential** (`key_env` on the credential), because it is the one part that is secret; the access key id is not | Refused at start-up on any kind but `bedrock`. `base_url` defaults to the regional runtime host. dorang speaks the **Converse** API (one request shape across model families) and signs with SigV4 — the signer is pinned to AWS's published test vector, not measured against a live account |
...
| `providers[].params.force_stream`, `providers[].params.store_false` | **New.** The contract of a host that serves `/responses` alone and refuses anything but `stream: true` + `store: false` — the ChatGPT Codex surface — stated once per provider instead of met by every caller as a 400. Routing to `/responses` is decided by the catalog kind's `responses_only` flag (single source: the adapter choice and the start-up check that refuses these two keys on any other kind read the same declaration), and a non-streaming caller still gets one buffered answer because the stream is collected into the neutral form and rendered by the caller's own encoder. Adversarial review of the first draft found the collected answer rendered on a side path — always chat JSON whatever the caller spoke, skipping the served-model note, the tool-argument check and the §10.5b transform, and passing a mid-stream error's text through unscrubbed with a retryable code that would have re-run a billed generation on a sibling deployment. All of that is now the shared `convert` tail, and each is a named test that fails under revert. A second round on the restructure found the rest: a transport failure under a forced stream was still `upstream_body` + retryable (now the bytes that arrived are read as the stream they are — a terminal event that got through is served, a cut-off generation is `upstream_stream_truncated`), `response.failed` decoded as a clean stop, a JSON error envelope on a 200 decoded as an empty success, dorang's own `previous_response_id` was serialised to a Responses upstream after the history it names had already been replayed, a `data:` payload that was not JSON was skipped and left a hole in the answer, refusal deltas were dropped on collection, the tool-name registry was not handed to this adapter, and `max_output_tokens` was not a drop alias for the one ceiling field. Each closed, each revert-verified. A third round found one blocker — an SSE event assembled across `data:` lines had no ceiling, so an upstream that never sent the blank line could grow one payload until the process died (now bounded by the frame limit) — and: a JSON document with `status: "failed"` decoded as a 200 with an odd stop (now `502 upstream_failed`, scrubbed, terminal, the buffered twin of the stream's failed event); an `event:` line inside a spec-ordered multi-line event dispatched half a document (now only a payload that is already whole); a byte-order mark stripped by the detector alone left the decoder refusing what the detector accepted (normalised once); a comment-opened stream was read as JSON (SSE comment prefix recognised again); a cut-off JSON document under a forced stream was run through the collector and offered to the chain as a wrong shape (the fall-through now applies only to a body that IS a stream; a cut-off document takes the rule every provider's takes); and the size ceiling was checked after a read error rather than before. Each revert-verified |
| `providers[].params.api_version`, kind `azure` | **New.** The azure kind was refused outright ("no adapter in this build"); it needed exactly two things beside the OpenAI adapter — the route and the header — and has them: the unified `/openai/v1` mount by default, the legacy per-deployment path with its mandatory `api-version` when the operator names one, and the `api-key` credential header. The kind's `api` is `azure-openai` now, so `max_tokens_field`, the T1 routes (embeddings, audio, images, moderations) and streaming legacy completions treat it as the OpenAI shape it is. `api_version` is refused off the kind, like every setting only one adapter reads |
| `providers[].params.project`, `providers[].params.location`, kind `vertex`, `oauth.format: gcp-service-account` | **New.** The vertex kind was refused outright: "it needs a project id, a location and a Google service-account OAuth exchange in a project-scoped path, none of which this configuration carries". It carries them now. The adapter is the Gemini one under the project-scoped route on the location's regional host; the credential is a service-account key file fitted into the OAuth credential's shape — `format: gcp-service-account` — so health, refresh-ahead, the 401 retry and the account header apply unchanged: the file is read-only (a refresh never rewrites the operator's key), the non-secret `private_key_id` stands in as the refresh token so the credential knows it can mint, and the first use mints an RS256 assertion and exchanges it (RFC 7523) synchronously. A store that holds no token at all mints on first use; an expired one stays with the refresh-ahead loop and the 401 path. `refresh.token_url` optionally overrides the file's `token_uri` and needs no `client_id`. Pinned against a fake Google token endpoint that verifies the signature, issuer, audience and scope, and against a fake Vertex host, from an operator's file |
| `providers[].params.region`, `.access_key_id`, `.session_token`, kind `bedrock` | **New.** The bedrock kind was refused: "it needs SigV4 request signing and the Converse request shape, neither of which dorang implements". Both are here: `internal/wire/bedrock` speaks Converse (the neutral form both mined sources named) and its binary event stream, and `internal/backend`'s SigV4 signer — pinned to AWS's own `get-vanilla` test vector — signs the built request through a new `requestSigner` seam in `send()`, because the credential is a signature over the whole request rather than a header. The secret access key is the credential; region and access key id are provider params; the session token is an optional secret ref. Not measured against a live account |
