...
> ⚠️ **"Team, user, and key limits all apply" was a design statement and, until 2026-08-03, not
> an implementation one (W12).** `auth.Principal` has always had `Key`, `User` and `Team`
> envelopes and `Authorize` has always enforced all three — but the one conversion that builds a
> principal from a stored row populated `Key` only, because the credential read joined
> `api_keys` and `api_key_secrets` and nothing else. `User` and `Team` were nil on every request
> the gateway has ever served, and every guard that reads them is nil-guarded, as it must be:
> an unowned key genuinely has neither. So the code was correct, the tests passed, and
> `users.blocked`, `teams.blocked`, both `max_budget_nano` columns, both rate ceilings and
> `teams.max_parallel` decided nothing at all.
>
> The fix is one line of SQL and one argument: the credential read LEFT-joins the owning user and
> team **in the same statement**, so §2.4's one-round-trip rule is intact, and `AuthPrincipal`
> takes the owners as a required argument rather than looking them up, so every construction site
> has to answer the question that went unasked. A tier is applied to the key's envelope only — a
> tier is granted to a credential (§11.6), and applying it to all three would narrow one grant
> three times.
>
> The general shape is worth naming because it is not the same as a missing feature: a struct
> field that no producer fills, read by a consumer that treats absence as permission, is
> indistinguishable from a working control at every layer except the one that would refuse a
> request. Neither the type checker nor a test that asserts the stored row can see it. The tests
> that hold it closed now assert a **refused request**.
...
The last column above is what this profile measured before the change described here. Nothing
about the architecture changed: it is still four passes, still `encoding/json`, still no new
dependency. Two pieces of pure duplication were removed from inside them.

**The Extra split was a second full parse per nesting level.** Every wire type with an `Extra`
map decoded its own bytes twice — once into the struct, and once more into a
`map[string]json.RawMessage` so the members the struct does not model survive the crossing
(§10.5a). Because *every* type did it, a chat request paid it at the request, at each message
and at each content part; and because `json.Unmarshal` into a map materializes the value of
every member, the largest thing it copied was `messages` — nearly the whole body — which the
next line deleted again as a modelled key. `wirejson.SplitExtra` is now a structural walk of
the top level that skips modelled values and allocates only for members that survive: **0
allocations** on a body whose members are all modelled, against 20 allocations and a copy of
the body before.

**The top-level `json.Unmarshal` walked the body twice to hand it to a method that already
had it.** `json.Unmarshal(b, v)` where `v` decodes itself runs `checkValid` over the whole
document and then the decoder's `skip()` over it again, purely to slice out the bytes it was
passed, before calling `v.UnmarshalJSON` — which validates a third time. `wirejson.UnmarshalSelf`
validates once and calls the method. It is *not* `v.UnmarshalJSON(b)`: the differential fuzzer
found that dropping the validation makes `{"Model":A}` decode cleanly, because COMPATIBILITY
2.0's filter excises the case-colliding member before the malformed value is ever looked at,
and a body no other parser accepts must not become a body dorang forwards.

| | before | after |
|---|---|---|
| `openai.DecodeRequest`, 1 KiB | 31 MB/s, 143 allocs, 12.4 KB | **57 MB/s, 65 allocs, 6.4 KB** |
| `openai.DecodeRequest`, 32 KiB | 35 MB/s, 2 092 allocs | **62 MB/s, 1 042 allocs** |
| `openai.DecodeRequest`, 400 KiB | 36 MB/s, 25 112 allocs | **62 MB/s, 12 555 allocs** |
| one dispatched 4 KiB request | 391 allocs, 71 KB | **262 allocs, 55 KB** |
| throughput ceiling (§15.3) | 20 489 req/s | **23 532 req/s** |

Both are pinned by differential tests rather than by argument — `wirejson`'s
`FuzzSplitExtra` and each adapter's `FuzzDirectDecode` run the new form and the one it
replaced over arbitrary bytes and require the same members, the same values and the same error
...
not. Each was a passing assertion that proved nothing. **All three are closed; the third was the oldest thing open on this page until 2026-09-12.**
...
| W12 | **The three-subject authorization envelope carried one subject** (§11.2) | **closed** — the credential read now LEFT-joins the owning `users` and `teams` rows in the SAME statement, so `auth.Principal.User` and `.Team` are populated on the hot path and §2.4's one-round-trip rule holds; `cluster.AuthPrincipal` takes the owners as a REQUIRED argument, so a construction site cannot omit them silently. `internal/store` gained the `users`, `teams` and `team_members` code it never had, and `internal/app` wires the `Directory` and `BudgetStore` seams `internal/admin` declared, so `/user/*`, `/team/*` and `/budget/*` stop answering 501. Measured: a user block reaches a second node in **19.6 ms** at `poll: 20ms` against the published 270 ms, and zero window on the node that took the call. `/model/*` stays 501 on purpose — `deployments` and `model_aliases` have a schema and no READER, since routing is compiled from configuration, so an adapter over them would write a row that changes no routing. Was **not recorded at all**, which is the point: `users.blocked`, `teams.blocked`, both `max_budget_nano` columns, both rate ceilings and `teams.max_parallel` reached no decision on any node at any latency, and nothing failed — every guard that reads an owner envelope is nil-guarded, correctly, because an unowned key has none. A field no producer fills, read by a consumer that treats absence as permission, is invisible to the type checker and to any test that asserts the stored row |
