...
`x-dorang-allow-lossy`에 나열한 경우에만 통과한다 — 그 경우 요청은 처리되고 잃은 구조는
`x-dorang-downgraded`에 이름으로 실린다. 이 헤더는 호출자가 언젠가 허용해 둔 손실이 아니라
**이번 요청이 실제로 잃은 것**을 보고한다.
...
- **아무 일도 하지 않는 값은 거부하지 않는다.** `n: 1`과 `service_tier: auto`는 "기본값을
  ...
  결함보다 나쁜 결함이다.
- **거부는 의도적으로 두 번, 하나의 집합에 대해 일어난다.** 라우팅은 "이 모델의 어떤 배포가
  표현할 수 있는가"에 답하고(DESIGN §7.1), 백엔드는 "선택된 그 배포가 표현할 수 있는가"에 —
  DESIGN §4.4의 자체 호스팅 정규화가 끝난 뒤에 — 답한다. 두 게이트는 *같은* 능력 값을 읽는다.
  라우팅 테이블에서 결정으로, 결정에서 인코더로 실려 내려간 그 값이다. 예전에는 각자 독립적으로
  계산했고, 하나의 질문에 두 개의 답이 있으면 **보이지 않는 방향으로** 실패한다: 라우팅은
  받아들이고 인코딩은 버리며, 호출자는 `200`을 받는다.
- **표현할 수 있는데도 보내지 않는 구조가 있다.** 자체 호스팅 엔진의 와이어 형태는
  `service_tier`를 받는다. 그래도 §4.4는 그것을 비운다. 고를 가격대가 없기 때문이다. 이것은
  능력 부재가 아니고 거부 대상도 아니다 — 요청은 처리되고 `service_tier`는
  `x-dorang-dropped-params`에 열거된다. DESIGN §10.5가 dorang 자신의 우선순위 클래스를 그
  필드에 넣어 호출자의 값이 와이어에 닿지 않는 경우도 같다. 둘 중 어느 쪽이든 능력 부재로
  모델링하면 **그 배포에 대해 사실이 아닌 이유로** `400`이 나간다.

#### 7.9a 응답 방향의 손실, 그리고 보고할 수 없는 한 가지 경우

위의 모든 것은 요청에 관한 것이다. 변환은 반대 방향으로도 잃는 것이 있고, 그쪽 손실은 요청에서
**유도되지 않는다**: 호출자가 선택지 네 개를 요구한 것이 아니라 업스트림이 네 개를 준 것이고,
호출자가 `pause_turn`을 요구한 것이 아니라 모델이 그렇게 멈춘 것이다. 요청 방향 보고를 만드는
능력 뺄셈은 *요청이 사용하는 것*에 대해 빼기 때문에 이 중 어느 것도 볼 수 없다.

**버퍼된 응답은 요청 방향 손실과 똑같이 보고한다.** 구조체 이름은 같은 응답의
`x-dorang-downgraded`와 `x-dorang-dropped-params`에 같은 규칙으로 실린다. 응답 바이트가
렌더링되는 시점에 헤더 블록이 아직 열려 있기 때문이다. 어휘도 위와 같다 — 교차 변환에서는
`rich_stop_reason`과 `thinking_block`이 흔하고, `n`은 드롭된 파라미터가 아니라 **다운그레이드**로
올린다. 선택지 네 개를 메시지 하나로 접는 것은 "노브 하나를 적용하지 않은 같은 답"이 아니라
"다른 답"을 건네는 일이기 때문이다. 요청 쪽이 `n`에 매기는 분류와 같으며, 둘이 어긋나 있던 것은
구분이 아니라 결함이었다.

**스트리밍 응답은 보고하지 않으며, 할 수 없다.** SSE 응답의 헤더는 첫 이벤트와 함께 나가고,
스트림의 응답 방향 손실은 이후 이벤트가 도착하면서 밝혀진다 — 접힌 stop reason은 종료 프레임에서야
알 수 있다. 그때는 남은 헤더 블록이 없고, 손실을 미리 아는 더 이른 지점도 없다. 패밀리 조합만으로
예측하면 실제로 잃은 것이 없는 교차 스트림까지 전부 다운그레이드로 표시하게 된다.

따라서 보장은 언급되지 않은 예외를 가진 규칙이 아니라 있는 그대로 진술한다:

> **`x-dorang-downgraded`와 `x-dorang-dropped-params`는 모든 응답에서 요청 방향을 보고하고,
> 응답 방향은 버퍼된 응답에서만 보고한다.**

스트림의 응답 방향 손실을 오늘 나르는 것은 아무것도 없으며, 이는 사실이 아닌 메커니즘으로 덮는
대신 공백으로 기록한다. 스트림에 아직 남아 있는 통로는 in-band 프레임 — 보고가 아니라 와이어
형태의 변경 — 과 원장뿐이고, 원장은 교환이 끝난 뒤 응답의 크리티컬 패스 밖에서 기록된다. 둘 다
각자의 근거로 결정할 일이고, 어느 쪽도 헤더는 아니다.
