...
// authenticate resolves the caller and enforces the one authorization rule
// §2.3 has: the out-of-band master credential, or a key with an administrative
// role. There is no per-endpoint permission model, because inventing one that
// the schema cannot express would be a permission model in name only.
