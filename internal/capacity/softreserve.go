...
//
// A claim is one unit of one axis key set aside for one waiter. It lives in two
// places that must agree: bucket.claimant, which is what every capacity check
// consults, and waiter.claims, which is how the waiter finds its claims again in
// order to give them back. The pair is only ever mutated under Broker.mu.
...
// ones on each axis, both axes saturated — against the same benchmark with the
// guard off:
//
//	arm after 1 bounce   +12.6%  ns/op
//	arm after 2 bounces   +6.0%
//	arm after 4 bounces   +3.1%   <- default
//	arm after 8 bounces   +0.8%
//
...
// holding n.
//
// The batch class is not consulted because batch waiters never reach here (see
// doc.go): for interactive work the effective ceiling is the raw limit, which is
// what makes the counting invariant inUse+claims <= limit hold against
...
func (b *Broker) claimableLocked(bk *bucket, n axisNeed, w *waiter) bool {
	...
	// occupancy test. The effective ceiling here is the raw limit because batch
	// waiters never claim.
	if bk.inUse >= n.limit {
...
func (b *Broker) extendClaimsLocked(w *waiter, on *bucket) bool {
	if !b.soft || w.req.Batch || w.bounces < int32(b.softAfter) {
		return false
	}
