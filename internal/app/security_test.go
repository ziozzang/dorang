...
func TestRPMLimitIsEnforced(t *testing.T) {
	...
	p := &principal{
		...
		rates: newKeyRates(func() time.Time { return now }),
	}
...
func TestTPMLimitIsEnforced(t *testing.T) {
	...
	// Settlement is where the token count exists. App.recordMetrics is the one
	// site that does this in production, from the meter event's subject ids.
	rates.recordTokens("key-1", "", "", 150)
...
func TestMaxParallelReachesThePrincipal(t *testing.T) {
	...
	// And the value reaches the routing request, which is the joint that was
	// missing: the column was stored, imported and administered, and nothing
	// carried it to the broker.
	var rr router.Request
	applyPrincipalPolicy(newDecodeRequest(t, `{"model":"m1","messages":[]}`, p), &rr)
	if rr.PrincipalMax != 2 {
		t.Errorf("router.Request.PrincipalMax = %d, want 2", rr.PrincipalMax)
	}
}
...
func TestRouterRequestCarriesTheTenant(t *testing.T) {
	...
	c, err := d.decode(context.Background(), d.state(), rq)
	if err != nil {
...
// The [HIGH] unbounded upstream read is closed one layer down, in
// internal/backend, which is the package that now makes the HTTP call:
// TestOversizedUpstreamResponseIsRefused there bounds the READ (it counts what
// the reader was asked for, because an implementation that reads the whole body
// and then measures it produces the same error and none of the safety), pins
// upstream_response_too_large, pins that it is not retryable, and asserts the
// shipped default is a real ceiling. This package held a second copy of that
// read and a second copy of that test while it had its own client; both went
// with the client rather than staying behind as the version nothing calls.
...
func newTestBatchExecutor(t *testing.T, baseURL string, owner *auth.Principal,
	...
	table := &upstreamTable{
		providers: map[string]*backend.Provider{"p1": testProvider(t, baseURL)},
...
func TestAHostileUpstreamCannotEchoTheCredentialBack(t *testing.T) {
	...
	table := &upstreamTable{
		providers: map[string]*backend.Provider{"p1": testProvider(t, up.URL)},
		creds: map[string]*credential{
...
func TestUnreachableUpstreamDoesNotNameInternalHosts(t *testing.T) {
	table := &upstreamTable{
		...
		providers: map[string]*backend.Provider{
			"p1": testProvider(t, "http://vllm-a.internal.invalid:8000"),
		},
...
// testProvider is one openai-kind provider pointed at a test server.
func testProvider(t *testing.T, baseURL string) *backend.Provider {
	t.Helper()
	p, err := backend.NewProvider(backend.Spec{
		Name: "p1", Kind: "openai", API: catalog.APIOpenAIChat, BaseURL: baseURL,
	})
	if err != nil {
