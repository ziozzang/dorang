...
func (e *batchExecutor) Execute(ctx context.Context, req *batch.ExecRequest) (*batch.ExecResult, error) {
	...
	defer func() {
		if !settled {
			// Nothing was spent: no answer came back.
			hold.release()
		}
	}()
	...
	if resp.StatusCode >= 400 {
		// A refusal cost nothing, so the hold is released rather than settled.
		return &batch.ExecResult{StatusCode: resp.StatusCode, Body: body}, nil
	}
	out, _, usage, err := e.d.convertResponse(c, dec, up, body, resp.Header)
	if err != nil {
		return nil, err
	}
	settled = true
	hold.settle(e.d.rowCost(st, dec, usage, now))
