...
// ReplaceKeyVerifier rotates the secret behind an existing key id.
//
// Every authorization field survives — models, budgets, limits, team, expiry —
// because rotating a credential must not silently widen or narrow what it can
// do. Only the three verifier columns and the label change, and the label
// changes BECAUSE it is derived from the secret: a label that outlived its
// secret would name a credential that no longer exists.
//
// The cutover is immediate. There is no grace window in which the old secret
// still verifies: this is the path an operator reaches for when a credential
// has leaked, and a grace window keeps a compromised secret working for its
// duration. A planned rotation that wants overlap issues a second key and
// retires the first.
func (s *Store) ReplaceKeyVerifier(ctx context.Context, id, lookup, digest string,
	scheme HashScheme, label string, now time.Time) error {

	if id == "" || lookup == "" || digest == "" {
		return errors.New("store: ReplaceKeyVerifier requires an id, a lookup and a digest")
	}
	if now.IsZero() {
		now = s.now()
	}
	const q = `UPDATE api_keys SET lookup = ?, token_hash = ?, hash_scheme = ?,
		key_label = ?, updated_at = ? WHERE id = ?`
	res, err := s.exec(ctx, q, lookup, digest, string(scheme), label, Micros(now), id)
	if err != nil {
		return err
	}
	n, err := res.RowsAffected()
